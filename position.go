...
import (
	...
	ransi "github.com/muesli/reflow/ansi"
...
func PlaceOverlay(x, y int, fg, bg string, opts ...WhitespaceOption) string {
	return renderer.PlaceOverlay(x, y, fg, bg, opts...)
}

// PlaceOverlay places fg on top of bg.
// Based on https://github.com/charmbracelet/lipgloss/compare/master...buztard:lipgloss:feature/overlay (with light edits)
func (r *Renderer) PlaceOverlay(x, y int, fg, bg string, opts ...WhitespaceOption) string {
...
func cutLeft(s string, cutWidth int) string {
	...
	for _, c := range s {
		var w int
		if c == ransi.Marker || isAnsi {
			isAnsi = true
			ab.WriteRune(c)
			if ransi.IsTerminator(c) {
				isAnsi = false
